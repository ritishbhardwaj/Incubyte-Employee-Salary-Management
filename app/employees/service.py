...
def filter_options(db: Session) -> dict[str, list[str]]:
    countries = list(
        db.scalars(select(Employee.country).distinct().order_by(Employee.country)).all()
    )
    departments = list(
        db.scalars(select(Employee.department).distinct().order_by(Employee.department)).all()
    )
    levels = list(
        db.scalars(select(Employee.job_level).distinct().order_by(Employee.job_level)).all()
    )
    return {
        "countries": countries,
...
def employee_to_detail(db: Session, employee: Employee) -> dict:
    from app.compensation.service import get_current_compensation
...
def apply_employee_filters(
    ...
    *,
    ...
    """Unpaginated filtered rows for CSV export."""
    descending = sort.startswith("-")
    key = sort[1:] if descending else sort
    if key not in SORT_FIELDS:
        raise ValidationAppError(f"sort must be one of: {', '.join(SORT_FIELDS)}")
