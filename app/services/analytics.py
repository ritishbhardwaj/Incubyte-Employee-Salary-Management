...
from app.database.models import CompensationRecord, Employee, EmploymentStatus
...
def _percentiles(db: Session) -> dict:
    ...
    from app.services.pg_percentiles import percentile_cont_usd
...
def distribution(db: Session) -> list[dict]:
    ...
    rows = {
        ...
        for row in db.execute(
            select(bucket_expr.label("label"), func.count().label("headcount")).group_by(bucket_expr)
        ).all()
    }
    return [
        {
